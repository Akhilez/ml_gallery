                ...
                <Hidden mdDown implementation="css">
                    <Sidenav style={{
                        ...
                        color: "gray"
                             ...
                             appearance={"subtle"} defaultOpenKeys={[`${openId}`]}>
                        <Sidenav.Header>
                            ...
                            <Nav>
                                {
                                    projects.categories.map((category, indexOuter) =>
                                        <Dropdown eventKey={`${indexOuter}`} title={category.title} key={indexOuter}
                                                  style={{color: (openId === indexOuter) ? "black" : "gray"}}>
                                            {category.projects.map((project, index) =>
                                                <Dropdown.Item eventKey={`${project.id}`} href={project.links.app}
                                                               key={project.id}>
                                                    <p style={{
                                                        margin: -5,
                                                        marginLeft: 10,
                                                        fontSize: 16,
                                                        color: (this.props.activeKey === project.id) ? "black" : "gray"
