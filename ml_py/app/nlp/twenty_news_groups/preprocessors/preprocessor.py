...
class Preprocessor:
    ...
    def make_csv(self, data_input_path, data_output_path, use_saved_tokenizer=False):
        dataset = text_dataset_from_directory(data_input_path, label_mode='categorical', batch_size=self.batch_size,
                                              max_length=self.max_char_length, shuffle=True)
        with open(data_output_path, 'w') as data_file:
            ...
            if self.tokenizer_type == BERT:
                ...
                if use_saved_tokenizer:
                    sequencer.tokenizer = CustomSequencer.load_tokenizer(self.tokenizer_path)
                else:
    ...
    def _prepare_and_write(dataset, sequencer, csv_writer):
        for batch in dataset:
            ...
            csv_writer.writerows(
                list(sequence) + list(label.numpy().astype(int)) for sequence, label in zip(sentences, labels))
